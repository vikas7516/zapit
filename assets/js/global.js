...
class ZapitCategoryPage {
    ...
    setupToolCards() {
        ...
        const categoryEl = document.querySelector('.category-title');
        ...
        const CATEGORY_SLUGS = {
            ...
            'Developer & Web Tools': 'developer-web-tools',
            'Security & Network Tools': 'security-network-tools',
            'Audio & Video Tools': 'audio-video-tools',
            'Miscellaneous & Fun Tools': 'miscellaneous-fun-tools'
        };
        const catSlug = CATEGORY_SLUGS[category] || (category || '').toLowerCase().replace(/&/g,'and').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'');
        // Normalize hrefs to slug URLs for SEO and copy-link correctness
